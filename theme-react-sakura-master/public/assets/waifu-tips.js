...
function loadTipsMessage(result) {
   ...
   // });
	...
	$.each(result.click, function (index, tips){
        $(document).on("mousedown", tips.selector, function (){
            var text = getRandText(tips.text);
            text = text.render({text: $(this).text()});
            showMessage(text, 3000, true);
			$('.waifu').css('cursor','grabbing')
        });
    });

	$.each(result.click, function (index, tips){
        $(document).on("mouseup", tips.selector, function (){
			$('.waifu').css('cursor','grab')
        });
    });
    ...
    window.showWelcomeMessage = function(result) {
        var text;
        if (window.location.href.substring(window.location.href.length -1) == live2d_settings.homePageUrl) {
			
            var now = (new Date()).getHours();
            if (now > 23 || now <= 5) text = result.waifu.hour_tips.t23_5[0];
            else if (now > 5 && now <= 7) text = result.waifu.hour_tips.t5_7[0];
            else if (now > 7 && now <= 11) text = result.waifu.hour_tips.t7_11[0];
            else if (now > 11 && now <= 14) text = result.waifu.hour_tips.t11_14[0];
            else if (now > 14 && now <= 17) text = result.waifu.hour_tips.t14_17[0];
            else if (now > 17 && now <= 19) text = result.waifu.hour_tips.t17_19[0];
            else if (now > 19 && now <= 21) text = result.waifu.hour_tips.t19_21[0];
            else if (now > 21 && now <= 23) text = result.waifu.hour_tips.t21_23[0];
            else text = result.waifu.hour_tips.default[0];
